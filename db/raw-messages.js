...
const { DEFAULT_RAW_DB_PATH } = require('./raw-db');
  ...
  try {
    ...
    return [...merged.values()].map((account) => {
      ...
      return {
        ...
        send_enabled: profile ? Number(profile.send_enabled) : 0,
...
function unreadGroupKey(platform, account, groupId) {
  return `${normalizePlatform(platform)}:${String(account || 'default')}:${String(groupId || '')}`;
}

function countUnreadForGroups({
  rawDbPath = DEFAULT_RAW_DB_PATH,
  groups = [],
  accountScope,
} = {}) {
  ...
  groups.forEach((group) => {
    ...
    const account = String(group.account || 'default');
    const groupId = String(group.group_id || group.groupId || '');
    if (!groupId || !accountScopeContains(accountScope, platform, account) || !isWorkbenchPlatform(platform)) return;
    const key = unreadGroupKey(platform, account, groupId);
    if (seen.has(key)) return;
    seen.add(key);
    targets.push({
      key,
      ...
      account,
      groupId,
      lastReadMessageId: Number(group.last_read_message_id || group.lastReadMessageId || 0) || 0,
    });
  });

  const result = new Map(targets.map((target) => [target.key, 0]));
  ...
  const values = targets.map((target, index) => {
    ...
    const groupIdKey = `groupId${index}`;
    const lastReadKey = `lastRead${index}`;
    params[platformKey] = target.platform;
    params[accountKey] = target.account;
    params[groupIdKey] = target.groupId;
    params[lastReadKey] = target.lastReadMessageId;
    return `(@${platformKey}, @${accountKey}, @${groupIdKey}, @${lastReadKey})`;
  });

  try {
    const rows = db.prepare(`
      WITH target(platform, account, group_id, last_read_message_id) AS (
        VALUES ${values.join(', ')}
      ),
      normalized AS (${normalizedMessagesSql(db)})
      SELECT
        target.platform,
        target.account,
        target.group_id,
        COUNT(normalized.id) AS count
      FROM target
      LEFT JOIN normalized
        ON normalized.platform = target.platform
       AND normalized.account = target.account
       AND normalized.group_id = target.group_id
       AND normalized.id > target.last_read_message_id
       ${INBOUND_MESSAGE_FILTER_SQL}
      GROUP BY target.platform, target.account, target.group_id
    `).all(params);
    rows.forEach((row) => {
      result.set(unreadGroupKey(row.platform, row.account, row.group_id), Number(row.count || 0));
    });
    return result;
  } finally {
    db.close();
