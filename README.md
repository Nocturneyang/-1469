...
客服工作台是独立部署、独立登录、独立存储、独立权限配置和独立前端的日常 IM 作业系统，用于客服或业务坐席接入 WA/TG 服务账号、查看会话、按授权范围回复消息、维护客户资料，并记录工作台侧的操作数据。
...
- 工作台负责：工作台登录身份、权限、WA/TG 服务账号登录入口、会话列表、消息线程、回复框、已读、客户资料、内部备注、外发账本和工作台标签。
...
全局真实发送和新登录服务账号均默认关闭。管理员完成授权后，必须同时显式开启 `WORKBENCH_SEND_ENABLED=1` 与账号自身的 `send_enabled`；任一级关闭都不会消费外发任务。
...
WA Web HTML 缓存默认按账号隔离并保留 72 小时。缓存过期后，账号 worker 在下一次启动时获取当前 WA Web 版本；旧缓存文件和账号 session 均保留。需要回退时可设置 `WORKBENCH_WA_WEB_VERSION=<version>`，需要忽略有效缓存刷新一次时可设置 `WORKBENCH_WA_WEB_CACHE_FORCE_LATEST=1`。会话列表同步失败按 30 秒起步、10 分钟封顶进行指数退避，不影响 worker 心跳、消息接收和外发消费循环。
