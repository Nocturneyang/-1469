...
async function readNativeWhatsAppChatSnapshot(client) {
  ...
  try {
    return await client.pupPage.evaluate(async () => {
      ...
      const groupParticipantModels = (groupMetadata) => {
        const source = groupMetadata?.participants;
        ...
        try {
          const models = source?.getModelsArray?.();
          if (Array.isArray(models)) return models;
        } catch (_) { }
        return Array.isArray(source?._models) ? source._models : [];
      };
      const refreshGroupMetadata = async (chat) => {
        const serializedId = chatId(chat);
        if (!chat?.groupMetadata && !serializedId.endsWith('@g.us')) return null;
        const metadataCollection = collections?.GroupMetadata
          || collections?.WAWebGroupMetadataCollection;
        try {
          if (typeof metadataCollection?.update === 'function') {
            const widFactory = window.require('WAWebWidFactory');
            const wid = widFactory?.createWid?.(serializedId) || chat?.id || serializedId;
            await metadataCollection.update(wid);
          }
        } catch (_) { }
        try {
          return chat?.groupMetadata
            || metadataCollection?.get?.(chat?.id)
            || metadataCollection?.get?.(serializedId)
            || null;
        } catch (_) {
          return chat?.groupMetadata || null;
        }
      };
      let contacts = [];
      try {
      ...
      const refreshedGroupMetadata = await Promise.all(sourceChats.map((chat) => refreshGroupMetadata(chat)));
      const models = sourceChats.map((chat, chatIndex) => {
        const id = chat?.id || {};
        const serializedId = chatId(chat);
        ...
        const groupMetadata = refreshedGroupMetadata[chatIndex] || chat?.groupMetadata || storedGroupMetadata;
        ...
        const formattedTitle = titleCandidates.find((value) => value && !isInternalId(value) && !isPhoneDisplay(value))
          ...
          || serializedId;
        const participants = isGroup ? groupParticipantModels(groupMetadata)
          .map((participant) => {
            const nativeParticipantId = chatId({ id: participant?.id || participant });
            if (!nativeParticipantId) return null;
            const alternateId = alternateContactId(nativeParticipantId);
            const participantId = alternateId || nativeParticipantId;
            const participantContact = findContact(
              { id: participant?.id || participant },
              nativeParticipantId,
            ) || (alternateId ? findContact({ id: alternateId }, alternateId) : null);
            const participantDetails = contactDetails(participantContact);
            return {
              id: participantId,
              name: participantDetails.name
                || normalizedText(participant?.name)
                || normalizedText(participant?.pushname)
                || participantDetails.phoneNumber
                || participantId,
              phone_number: participantDetails.phoneNumber
                || normalizedText(alternateId).split('@')[0]
                || normalizedText(participantId).split('@')[0],
              lid_id: nativeParticipantId.endsWith('@lid') ? nativeParticipantId : '',
              phone_id: alternateId || (/@(?:c\.us|s\.whatsapp\.net)$/i.test(participantId) ? participantId : ''),
...
function toWhatsAppGroup(chat, options = {}) {
  ...
  const participants = sourceParticipants.map((participant) => {
    ...
    return {
      ...
      lid_id: String(participant?.lid_id || (id.endsWith('@lid') ? id : '')).trim(),
      phone_id: String(participant?.phone_id || (/@(?:c\.us|s\.whatsapp\.net)$/i.test(id) ? id : '')).trim(),
      is_admin: Boolean(participant?.isAdmin || participant?.admin),
      is_super_admin: Boolean(participant?.isSuperAdmin || participant?.superAdmin),
    };
  }).filter(Boolean).slice(0, 1024);
