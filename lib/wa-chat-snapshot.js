  ...
  try {
    return await client.pupPage.evaluate(() => {
      ...
      const models = sourceChats.map((chat) => {
        ...
        const titleCandidates = chat?.groupMetadata
...
function whatsappDisplayName(chat, contact, fallbackId = '') {
  ...
  return candidates.find((value) => value && !isWhatsAppInternalId(value) && !isWhatsAppPhoneDisplay(value))
    || candidates.find((value) => value && !isWhatsAppInternalId(value))
    || groupId
    || '未命名会话';
}
