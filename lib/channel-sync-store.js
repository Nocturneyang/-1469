...
function replaceChannelSnapshot({
  ...
  try {
    return withSqliteBusyRetry(() => workbenchDb.transaction(() => {
...
function withSqliteBusyRetry(operation, attempts = 5) {
...
function buildServiceGroups(platform, account, groups, labels, shouldSyncLabels) {
  if (shouldSyncLabels) {
    const source = platform === 'wa' ? 'wa_label' : 'tg_folder';
...
function normalizeLabels(labels, platform, account) {
  ...
  return (labels || [])
    .map((label) => ({
      ...
      name: normalizeChannelLabelName(label.name ?? label.title, label.id),
...
function normalizeChannelLabelName(value, fallback = '') {
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'bigint') {
    return String(value).trim() || String(fallback || '').trim();
  }
  if (value && typeof value === 'object') {
    for (const candidate of [value.text, value.name, value.title]) {
      const normalized = normalizeChannelLabelName(candidate, '');
