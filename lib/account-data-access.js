...
function createAccountDataAccess({
  ...
  function withWorkbenchDb(platform, account, options = {}, fn) {
    ...
    const paths = options.create
      ? ensureAccountDatabases(platform, account, { accountDataDir })
      : resolveAccountPaths(platform, account, { accountDataDir });
    const db = openWorkbenchDb(paths.workbenchDbPath, { readonly: Boolean(options.readonly) });
    try {
  ...
  function accountRuntimeActive(platform, account) {
    return withRuntimeDb(platform, account, { create: false }, (runtimeDb) => {
      const lease = runtimeDb.prepare(`
        SELECT holder_id, run_id, pid, renewed_at, expires_at
        FROM account_worker_leases
        WHERE platform = @platform
          AND account = @account
          AND lease_name = 'account-runtime'
      `).get({ platform: normalizeAccountPlatform(platform), account: String(account || '').trim() });
  ...
  function requestServiceAccountLogout({ platform, account } = {}) {
    const normalizedPlatform = normalizeAccountPlatform(platform);
    const normalizedAccount = String(account || '').trim();
    const dbPath = rawDbPathFor(normalizedPlatform, normalizedAccount);
    const requested = setServiceAccountStatus({
      dbPath,
    ...
    const lease = accountRuntimeActive(normalizedPlatform, normalizedAccount);
    if (lease) {
      return {
        platform: normalizedPlatform,
        account: normalizedAccount,
    ...
    const session = clearServiceAccountSession(normalizedPlatform, normalizedAccount, {
      isolated,
      accountDataDir,
    });
    setServiceAccountStatus({
  ...
  function openAccountRawDb(platform, account, options = {}) {
    if (!isolated) return options.readonly ? rawMessages.openRawDb(legacyRawDbPath) : ensureRawDb(legacyRawDbPath);
    const paths = options.create
      ? ensureAccountDatabases(platform, account, { accountDataDir })
      : resolveAccountPaths(platform, account, { accountDataDir });
    return options.readonly ? rawMessages.openRawDb(paths.rawDbPath) : ensureRawDb(paths.rawDbPath);
  }
  ...
  function listAccountProfiles(options = {}) {
    ...
    filterRefs(options).forEach((ref) => {
      ...
      const runtimeStatus = readRuntimeStatus(ref);
      rows.push(...rawMessages.listAccountProfiles({
        rawDbPath: ref.paths.rawDbPath,
        accountScope: accountScopeFor(ref),
      }).map((profile) => runtimeStatus === 'capacity_waiting' ? { ...profile, status: runtimeStatus } : profile));
    });
    return dedupeByAccount(rows);
  }

  function readRuntimeStatus(ref) {
    if (!fs.existsSync(ref.paths.runtimeDbPath)) return '';
    const db = openRuntimeDb(ref.paths.runtimeDbPath);
    try {
      return String(db.prepare(`
        SELECT status FROM account_worker_status WHERE platform = ? AND account = ?
      `).get(ref.platform, ref.account)?.status || '');
    } finally {
      db.close();
    }
  }
  ...
  function listGroups(options = {}) {
    if (!isolated) return rawMessages.listGroups({
      rawDbPath: legacyRawDbPath,
      ...options,
      groupIds: options.groupIdsByAccount instanceof Map
        ? [...(options.groupIdsByAccount.get('legacy') || [])]
        : options.groupIds,
    });
    const limit = boundedNumber(options.limit, 80, 1, 500);
    const offset = Math.max(0, Number(options.offset) || 0);
    const perAccountLimit = Math.min(limit + offset, 500);
    const rows = [];
    filterRefs(options).forEach((ref) => {
      if (!fs.existsSync(ref.paths.rawDbPath)) return;
      const targetGroupIds = options.groupIdsByAccount instanceof Map
        ? [...(options.groupIdsByAccount.get(`${ref.platform}:${ref.account}`) || [])]
        : undefined;
      if (options.groupIdsByAccount instanceof Map && !targetGroupIds.length) return;
      rows.push(...rawMessages.listGroups({
        ...
        limit: Math.max(perAccountLimit, targetGroupIds?.length || 0),
        offset: 0,
      }));
    });
    return rows
  ...
  function listQuoteTexts(options = {}) {
    if (!isolated) return rawMessages.listQuoteTexts({ rawDbPath: legacyRawDbPath, ...options });
    return rawMessages.listQuoteTexts({
      ...options,
      rawDbPath: rawDbPathFor(options.platform, options.account),
      accountScope: accountScopeFor({ platform: options.platform, account: options.account }),
      directAccount: true,
...
function clearServiceAccountSession(platform, account, options = {}) {
  const normalizedPlatform = normalizeAccountPlatform(platform);
  const normalizedAccount = String(account || '').trim();
  if (options.isolated) {
    const paths = resolveAccountPaths(normalizedPlatform, normalizedAccount, {
      accountDataDir: options.accountDataDir,
    });
    const cleared = safeRmWithin(paths.accountDir, paths.sessionDir);
    fs.mkdirSync(paths.sessionDir, { recursive: true });
    return { cleared_session_paths: cleared };
  }
  const deleted = deleteLegacySessionFiles(normalizedPlatform, normalizedAccount, options);
  return { cleared_session_paths: Number(deleted.deleted_session_paths || 0) };
}
