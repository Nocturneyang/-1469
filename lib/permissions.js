...
function loadPortalAccess(db, operator = {}) {
  ...
  if (row) {
    return resolvePortalLanding({
      can_monitor: false,
      // A persisted portal row is an explicit allow/deny decision. In
      // particular, 0 must revoke access even if a role still grants it.
      can_workbench: Number(row.can_workbench) === 1,
      can_admin: Number(row.can_admin) === 1,
