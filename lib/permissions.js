const { resolveOrCreateOperator } = require('../db/workbench-db');
...
function resolveWorkbenchOperator(db, req) {
  ...
  const identityUser = user && !allowHeaderOperator ? user : null;
  const stableIdentities = identityUser
    ? [user.id, user.username, user.email, user.mobile].filter(Boolean)
    : [id, username];
  const operatorId = resolveOrCreateOperator(db, {
    id: id || 'demo-operator',
    username,
    displayName: displayName || id || 'demo-operator',
    identities: stableIdentities,
    source: identityUser ? 'sso' : 'local',
  });
...
function operatorIsActive(operator = {}) {
  return String(operator.status || 'active').trim().toLowerCase() === 'active';
}

function requireActiveWorkbenchOperator(db) {
  return (req, res, next) => {
    const operator = req.workbenchOperator || resolveWorkbenchOperator(db, req);
    req.workbenchOperator = operator;
    if (operatorIsActive(operator)) return next();
    return res.status(403).json({ success: false, ok: false, error: 'Workbench operator is disabled' });
  };
}
...
function requireWorkbenchPortalAccess(db) {
  return (req, res, next) => {
    const operator = req.workbenchOperator || resolveWorkbenchOperator(db, req);
    req.workbenchOperator = operator;
    if (!operatorIsActive(operator)) {
      return res.status(403).json({ success: false, ok: false, error: 'Workbench operator is disabled' });
    }
    const portal = loadPortalAccess(db, operator);
    if (portal.can_workbench || portal.can_admin) return next();
    return res.status(403).json({ success: false, ok: false, error: 'Workbench portal access is disabled' });
  };
}
...
function legacyRoleCodes(operator = {}) {
  ...
  const role = String(operator.role || operator.user && operator.user.role || '').trim().toLowerCase();
  ...
  if (['super_admin', 'workbench_manager', 'agent'].includes(role)) {
    return [role];
  }
  return [];
}
...
function loadPortalAccess(db, operator = {}) {
  ...
  if (!operatorIsActive(operator)) return { ...DEFAULT_PORTAL_ACCESS };
  ...
  const derived = {
    can_monitor: false,
    can_workbench: permissions.has('workbench:view'),
...
function resolvePortalLanding(access) {
  ...
  let landing = '/';
  const allowedCount = [canWorkbench, canAdmin].filter(Boolean).length;
...
function requireMonitorPortalAccess(db) {
  return (req, res, next) => {
    const operator = req.workbenchOperator || resolveWorkbenchOperator(db, req);
    req.workbenchOperator = operator;
    return res.status(404).json({ success: false, ok: false, error: 'Monitor portal is not part of Workbench' });
  };
}
...
function conversationHasCapability(db, operator, platform, account, groupId, capability = 'can_view', nativeGroupIds = null) {
  if (!operatorIsActive(operator)) return false;
