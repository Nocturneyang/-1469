...
function createServiceAccountLoginRequest({
  ...
  const credentialPayload = buildCredentialPayload(mode, secret, { tgApiId, tgApiHash });
...
function buildCredentialPayload(mode, secret, { tgApiId, tgApiHash } = {}) {
  if (!secret) return null;
  const payload = { mode, value: secret };
  if (mode !== 'tg_user_session') return payload;
