...
function assertChromeHeadlessLaunch(launchConfig, stateDirs, { env = process.env, log = () => {} } = {}) {
  ...
  const args = [
    ...launchConfig.args.filter((arg) => !String(arg).startsWith('--user-data-dir=')),
    '--headless',
  ...
  if (result.error) {
    throw new Error(`Chromium headless 预检失败：${result.error.message}`);
  }
  if (result.status !== 0 || result.signal) {
    const detail = compactProcessOutput(result);
    throw new Error(
      `Chromium headless 预检失败，退出码 ${result.status ?? 'null'}，signal ${result.signal || '-'}${detail ? `：${detail}` : ''}`
    );
  }
  log('chromium headless preflight ok');
}
